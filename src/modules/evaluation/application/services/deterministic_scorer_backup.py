    ...
    for u in units:
        try:
            ...
            if 0 <= page <= total_pages:
                ...
                warnings.append(f"Dropped out-of-bounds page reference: {page} (Max: {total_pages})")
                continue
        except (ValueError, TypeError):
            warnings.append(f"Dropped invalid page reference format: {u.slide_number_or_page_number}")
            continue
...
def calculate_contradiction_cap(severity: str) -> (float, float):
    if severity == "severe": return 5.0, 3.0
    if severity == "moderate": return 7.0, 1.5
    if severity == "mild": return 9.0, 0.5
    return 10.0, 0.0
...
class DeterministicScoringService:
    ...
    def score(
        ...
        # Simple dynamic weights based on stage (Idea vs Growth etc)
        weights = {
        ...
        ev_map: Dict[str, CriterionEvidence] = {(normalize_criterion_code(c.criterion) or c.criterion): c for c in evidence.criteria_evidence}
        ...
        for raw_j in raw_judgments.raw_judgments:
            ...
            if not ev:
                self.warnings.append(f"No evidence mapping found for criterion: {criterion}")
                continue
            ...
            valid_pages = sanitize_page_refs(ev.evidence_units, self.total_pages, self.warnings)
            ...
            cf = {"High": 1.0, "Medium": 0.5, "Low": 0.1}.get(raw_j.criterion_confidence, 0.5)
            ...
            final_criteria.append(FinalCriterionResult(
                criterion=criterion,
                status=status,
                raw_score=(raw_j.raw_score * 10.0) if raw_j.raw_score is not None else None,
                ...
                strengths=[u.excerpt_or_summary for u in ev.evidence_units[:2]],
                concerns=[u.excerpt_or_summary for u in ev.weakening_evidence_units[:2]] + ev.gaps[:2],
        ...
        if active_weights_sum > 0:
            ...
            overall_score = None
            self.warnings.append("No active criterion weights could be evaluated. Overall score is completely omitted.")

        # Band
        band = "weak"
        if overall_score:
            if overall_score >= 85: band = "very strong"
            elif overall_score >= 70: band = "strong"
            elif overall_score >= 50: band = "promising but incomplete"
            elif overall_score >= 35: band = "below average"

        overall_conf = "Medium"
        avg_conf = sum(confidences) / len(confidences) if confidences else 0
        if avg_conf > 0.75: overall_conf = "High"
        elif avg_conf < 0.3: overall_conf = "Low"
        ...
        return DeterministicScoringResult(
            ...
            overall_result=OverallResult(
                ...
                evidence_coverage="strong" if avg_conf > 0.6 else ("weak" if avg_conf < 0.3 else "moderate"),
